...
int vsock_socketpair_connectible(int sotype, int *v0, int *v1)
{
	struct sockaddr_storage addr;
...
static int vsock_socketpair_dgram(int *c, int *p)
{
...
static void vsock_redir_dgram(int sock_mapfd, int verd_mapfd, enum redir_mode mode)
{
	...
	if (vsock_socketpair_dgram(&c0, &p0) < 0)
		return;

	if (vsock_socketpair_dgram(&c1, &p1) < 0)
		goto close;
